 ...
 *
 * Originally based on postgres/query-generator.js
 * ========================================================= */
  /* createFunction returns a statement without a final delimiter (;),
   * allowing the delimiter to be changed. */
  createFunction(functionName, params, returnType, language, body, options) {
    ...
    // NB There must be no final delimiter, see mysql/query-interface
  ...
  expandFunctionParamList(params) {
    ...
    _.each(params, curParam => {
      ...
      if (_.has(curParam, 'type')) {
        ...
        // MashTheKeys: mysql parameters are IN parameters by default
